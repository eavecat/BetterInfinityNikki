...
public sealed class MemoryFileCache
{
    ...
    private static string ComputeHash(string cacheKey)
    {
        ...
        var hashBytes = md5.ComputeHash(Encoding.UTF8.GetBytes(cacheKey));
    ...
    private static byte[] ReadAllBytesByCopyTo(FileStream fs)
    {
        if (fs.Length <= 0 || fs.Length > int.MaxValue)
        {
            return [];
    ...
    private static void WriteAllBytesByCopyTo(FileStream fs, byte[] bytes)
    {
        if (bytes.Length <= 0)
        {
            return;
