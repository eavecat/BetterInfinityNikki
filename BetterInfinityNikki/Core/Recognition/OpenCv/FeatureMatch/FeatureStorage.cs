...
public class FeatureStorage
{
    ...
    public unsafe KeyPoint[]? LoadKeyPointArray()
    {
        ...
        if (File.Exists(kpPath))
        {
            ...
            var sizeOfKeyPoint = System.Runtime.InteropServices.Marshal.SizeOf<KeyPoint>();
            if (fs.Length % sizeOfKeyPoint != 0) throw new FileFormatException("无法识别的KeyPoint格式");
            using var kpVector = new OpenCvSharp.Internal.Vectors.VectorOfKeyPoint((nuint)(fs.Length / sizeOfKeyPoint));
            using var ms = new System.IO.UnmanagedMemoryStream((byte*)kpVector.ElemPtr, fs.Length, fs.Length, FileAccess.Write);
            fs.CopyTo(ms);
    ...
    public unsafe void SaveKeyPointArray(KeyPoint[] kpArray)
    {
        CreateFolder();
        var kpPath = Path.Combine(_rootPath, $"{_name}_{TypeName}.kp.bin");
        var kpVector = new OpenCvSharp.Internal.Vectors.VectorOfKeyPoint(kpArray);
        var sizeOfKeyPoint = System.Runtime.InteropServices.Marshal.SizeOf<KeyPoint>();
