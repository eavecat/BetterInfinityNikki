...
## 功能
* 自动拾取: 遇到可交互/拾取内容时自动按 F，支持黑白名单配置。支持萤火虫套和花套的范围采集。
* 自动剧情: 快速点击过剧情，暂不支持自动选择选项。
* 半自动钓鱼: 手动抛竿后，鱼上钩时自动收杆，并自动完成钓鱼进度。
* 地图遮罩层: 把官方大地图的资源点位渲染在地图遮罩层上，支持大地图、花焰群岛、无忧岛、丹青屿、丹青之境、万相境；暂不支持蜗牛城、分层地图。支持隐藏已获取点位(需要配置大地图网页工具的token)。
...
推荐的电脑配置至少能够中画质流畅游玩无限暖暖，否则部分功能的使用体验会较差。
...
**打开软件以后，在“启动”页选择好截图方式，点击启动按钮就可以享受 BetterIN 带来的便利了！**

获取网页大地图工具token：[官方大地图](https://myl.nuanpaper.com/tools/map)登录后按F12打开浏览器控制台，选择网络(Network)面板，然后刷新网页；找到网络请求列表中的list、get、info任意一个结尾的请求，复制请求负载中的token到BetterIN中即可。暂不支持多账号。

目前没有线上配套的后端服务，检测更新功能暂时不可用。
...
* 会不会封号？
  * 理论上不会被封。 **BetterIN 不会做出任何修改游戏文件、读写游戏内存等任何危害游戏本体的行为，单纯依靠视觉算法和模拟操作实现。** <!--但是mhy是自由的，用户条款上明确说明第三方软件/模拟操作是封号理由之一。当前方案还是存在被检测的可能。-->只能说请低调使用，请不要跳脸官方。
...
提 [Issue](https://github.com/eavecat/BetterInfinityNikki/issues) 
<!-- 或 QQ群[1045965909](https://qm.qq.com/q/v5yJPb63Ze) -->
